// c4_modified.c - Modified version of the C4 compiler with for loop and modulus support
...
enum {
  ...
  Char, Else, Enum, If, Int, Return, Sizeof, While, For,  // Keywords
...
void next() {
  ...
  while (tk = *p) {  // Loop through the source code
    ...
    else if (tk == '%') { tk = Mod; return; }  // Handle '%' (modulus operator)
...
void stmt() {
  ...
  else if (tk == For) {  // Handle for loops
    next();
    if (tk == '(') next(); else { printf("%d: open paren expected\n", line); exit(-1); }
    expr(Assign);  // Parse initialization
    if (tk == ';') next(); else { printf("%d: semicolon expected\n", line); exit(-1); }
    int *for_start = e + 1;  // Save start of condition check
    expr(Assign);  // Parse condition
    if (tk == ';') next(); else { printf("%d: semicolon expected\n", line); exit(-1); }
    *++e = BZ; int *for_exit = ++e;  // Emit branch-if-zero for condition
    int *for_update = e + 1;  // Save start of update
    expr(Assign);  // Parse update
    if (tk == ')') next(); else { printf("%d: close paren expected\n", line); exit(-1); }
    *++e = JMP; *++e = (int)for_start;  // Jump back to condition check
    stmt();  // Parse loop body
    *++e = JMP; *++e = (int)for_update;  // Jump to update
    *for_exit = (int)(e + 1);  // Set exit point
  }
